...
const proceduredb = new Datastore({ filename: 'maestro-procedure.db', autoload: true });
   ...
   proceduredb.findOne({ userId: req.sessionID }, function (err, doc) {
     if (_.isEmpty(doc)) {
       ...
       for (var p in doc.procedureDetails.tasks) {
         ...
         if ((_.size(actorKeys) < 2) && (doc.procedureDetails.tasks[p].roles.crewA === actor1)) {
           keycolumn1 = "crewA";
           keycolumn2 = "";
         }
         else if ((_.size(actorKeys) < 2) && (doc.procedureDetails.tasks[p].roles.crewA === actor2)) {
           keycolumn1 = "";
           keycolumn2 = "crewA";
         } else {
           keycolumn1 = "crewA";
         ...
         var obj1 = _.object(['file', 'column', 'position'], [taskfilename, keycolumn1, p]);
         var obj2 = _.object(['file', 'column', 'position'], [taskfilename, keycolumn2, p]);
     ...
     res.render('summary-timeline.html', { sessionDoc: doc , taskDoc : taskdoc, actor1: actor1procedure, actor2 : actor2procedure });
