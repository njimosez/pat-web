...
const patProjectData = function getProjectFolderData(
  userSessionDir,
  giturl,
  folderName,
  next,
  ...
  var projectFolderPath = getTree(
    userSessionDir + "/" + path.basename(giturl) + "/" + folderName
  );
  ...
  if (_.isEmpty(projectFolderPath)) {
    ...
    next(
      new Error(
        "This project " + folderName + " folder is empty or does not exist!"
      )
    );
  }
